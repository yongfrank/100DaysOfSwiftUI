...
//    }
//    
//    func animationGraph(onChanged: Bool = false) {
//        for (index, _) in sampleAnalytics.enumerated() {
//            DispatchQueue.main.asyncAfter(deadline: .now() + Double(index) * (onChanged ? 0.03: 0.05 * 0.05)) {
//                withAnimation(onChanged ? .easeInOut(duration: 0.8) : .interactiveSpring(response: 0.8, dampingFraction: 0.8, blendDuration: 0.7)) {
